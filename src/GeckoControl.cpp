...
void GeckoControl::flipperVelocity2mbedTxData(const gecko_msgs::FlipperVelocity& flipper_velocity, gecko_msgs::MbedTx* tx_data)
{
  ...
  tx_data->data[0] = static_cast<uint8_t>(flipper_velocity.front_left);
  tx_data->data[1] = static_cast<uint8_t>(flipper_velocity.rear_left);
  tx_data->data[2] = static_cast<uint8_t>(flipper_velocity.front_right);
  tx_data->data[3] = static_cast<uint8_t>(flipper_velocity.rear_right);
#ifdef DEBUG
  for (int i = 0; i<3; i++)
  {
